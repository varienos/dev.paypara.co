	...
	public function datatable()
	{
		...
		foreach ($data['dataTable']->getResult() as $row) {
			...
			$records["data"][$i] = array(
				...
				$row->api_key_pin . "-****-****-****-************",
				$row->private_key,
				'<div class="form-check form-switch form-switch-sm form-check-success form-check-custom form-check-solid flex-center"><input type="checkbox" role="switch" ' . (edit_firm !== true ? "disabled" : null) . ' id="flexSwitchCheckChecked" data-set="switch" data-id="' . $row->id . '" name="status" class="form-check-input h-20px w-45px" ' . $status . '></div>',
