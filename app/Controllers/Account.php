...
class Account extends BaseController
{
	...
	{
		$this->db->query("update account set status='" . $status . "' where " .
			($type != 0 ? "  dataType='" . $type . "' " : null) .
			($id > 0 && $type != 0 ? " and " : null) .
			($id > 0 ? " id='" . $id . "'" : null));
	...
	public function listMatch($account_id)
	{
		...
		if (count((array)$listMatch) > 0) {
			...
			echo '<tr>
            	<td class="dataTables_empty text-center" colspan="7">No records</td>
            </tr>';
		}
	}
	...
	public function listDisableMatch($account_id)
	{
		...
		$orderDir == "" ? $orderDir = "desc" : $orderDir = $orderDir;
