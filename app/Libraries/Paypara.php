...
class Paypara
{
    ...
    public function getGamerLastDepositTime($gamer_site_id)
    {
        $data = $this->db->query("
            SELECT request_time
            FROM finance
            WHERE gamer_site_id   ='" . $gamer_site_id . "' AND
            status ='onaylandı'
    ...
    public function getGamerMonthlyDeposit($gamer_site_id)
    {
        $data = $this->db->query("
            ...
            FROM finance
            WHERE MONTH(request_time) = MONTH(CURRENT_DATE()) AND
    ...
    {
        // MIN. LIMIT YATIRIMA UYGUN EŞLEŞTİRME HESAPLARINI DÖNDÜR
        ...
        if (count((array)$matchAccounts) > 0) {
            ...
            foreach ($matchAccounts as $row) {
                //EŞLEŞME LİMİTİ UYGUN HESABI BUL
                if ($this->accountTotalMatch($row->id) < $row->match_limit && $this->setNumber($this->accountTotalMonthlyDeposit($row->id)) < $this->setNumber($row->limitDeposit)) {
                    $this->setLog("setNewMatch", "EŞLEŞME VE AYLIK YATIRIM LİMİTİ UYGUN HESAP BULUNDU", $userId, $site_id, $deposit, $vip, $row->id, $transaction_id);
